 ...
 */
	...
	class RivetyCore_Amazon_SQS
	{
		...
		function RivetyCore_Amazon_SQS($key, $secret, $queue_url = null)
		{
			$this->_key    = $key;
			$this->_secret = $secret;
		...
		function sendMessage($message_body, $queue_url = null)
		{
			...
			if($xml === false){
				...
				RivetyCore_Log::report("sent message", $xml->SendMessageResult->MessageId,Zend_Log::INFO);
		...
		function receiveMessage($number = 1, $timeout = null, $queue_url = null)
		{
			...
			if($xml === false){
				RivetyCore_Log::report('sqs cannot recieve messages',null,Zend_Log::ERR);				
				return false;
			} else {
				RivetyCore_Log::report('sqs recieved messages',$xml,Zend_Log::INFO);
				$out = array();
