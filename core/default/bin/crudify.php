...
echo("Theme:".$theme."\n");
if ($is_admin) {
	$theme_dir = $basepath."/themes/admin/".$theme."/tpl_controllers/".$module_name."/".strtolower($name).'admin';
} else {
	$theme_dir = $basepath."/themes/frontend/".$theme."/tpl_controllers/".$module_name."/".strtolower($name);
}
if (!is_dir($theme_dir)) {
...
if ($is_admin) {
	$controller_class->setName($controller_name)->setExtendedClass("RivetyCore_Controller_Action_Admin");
} else {
	$controller_class->setName($controller_name)->setExtendedClass("RivetyCore_Controller_Action_Abstract");
}
...
if (!is_array($pk)) {
	...
	$edit_method_body = RivetyCore_Common::replaceWithArray($edit_method_body, $edit_action_replacements);
	...
	$index_method_body = RivetyCore_Common::replaceWithArray($index_method_body, $index_action_replacements);
	...
	$delete_method_body = RivetyCore_Common::replaceWithArray($delete_method_body, $delete_action_replacements);	
