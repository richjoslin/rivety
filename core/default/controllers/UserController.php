...
*/
class UserController extends RivetyCore_Controller_Action_Abstract
{
	...
	*/
	...
	{
		parent::init();
		// $this->view->full_name_length = RivetyCore_Registry::get('full_name_length');
		$this->view->username_length = RivetyCore_Registry::get('username_length');
		$this->_users_table = new Users();
		if ($this->_auth->hasIdentity()) $default = $this->_identity->username;
		else $default = null;
	...
	*/
	...
	/*
		...
		View Variables:
			...
			// signs - An array of astrological star signs generated with RivetyCore_Common::GetSignArray.
	...
	*/
	function indexAction()
	{
		...
		{
	   		if (!in_array($key, $ignore) && strlen($val) > 0) $params[$key] = $val;
	   	}
		...
		$per_page = RivetyCore_Registry::get('users_per_page');
		...
		$params = $this->_rivety_plugin->doFilter($this->_mca . "_users", $params); // FILTER HOOK
		$users = $params['users'];
		...
		if (count($users) > 0)
		{
			...
			{
				// $user['sign'] = RivetyCore_Common::calculateAstroSign(strtotime($user['birthday']));
				...
				// if (!is_null($country)) $user['location'] = $country->country;
				$tmp_users[] = $user;
			}
	...
	*/
	function forgotpasswordAction()
	{
		...
		{
			$this->_redirect('/default/user/profile/'.$this->_identity->username);
		}
	...
	*/
	...
	{
		$request = new RivetyCore_Request($this->getRequest());
		$redirect_url = RivetyCore_Registry::get('default_postregister_redirect');
		$params = array('redirect_url' => $redirect_url, 'session' => $this->session);
		$params = $this->_rivety_plugin->doFilter($this->_mca . '_redirect', $params); // FILTER HOOK
		if ($request->has('url'))
		{
	...
	*/
	function profileAction()
	{
		if (!is_null($this->_user))
		{
			...
			$users_table = new Users();
			$users = $users_table->fetchByUsername($this->_username);
			$tmp_user = array_merge($tmp_user, $users->toArray());
	...
	*/
	function registerAction()
	{
		...
		if ($this->_auth->hasIdentity()) $this->_redirect('/default/user/profile/' . $this->_identity->username);
		$users_table = new Users();
		$user = array();
