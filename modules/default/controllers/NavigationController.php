...
*/
class NavigationController extends Cts_Controller_Action_Admin {
		...
		if ($request->has('role_id')) {
			...
			if ($this->getRequest()->isPost()) {
				...
				if ($nav_id == 0) {
					...
					Cts_Sort::reNumber('Navigation', "parent_id = " . $parent_id . " and role_id = " . $role_id, 'id', 'sort_order', 10, $params);
				} else {
					// UPDATE
					$nav_table->update($data, 'id = ' . $nav_id);
				}
				Cts_Cache::removeByTags(array('navigation'));
	...
	*/
	private function move($request, $adjustment) {
		if ($request->has('nav_id') && $request->has('role_id')) {
			...
			Cts_Sort::adjustSortValue($class_name, $nav_id, $adjustment, 'id', 'sort_order', $params);
			if ($request->has('parent_id')) {
				$params = array($role_id,$this->locale_code);
				Cts_Sort::reNumber($class_name, "parent_id = " . $request->parent_id . " and role_id = " . $role_id, 'id', 'sort_order', 10, $params);
			} else {
				$params = array($role_id,$this->locale_code);
				Cts_Sort::reNumber($class_name, "parent_id = 0 and role_id = " . $role_id, 'id', 'sort_order', 10, $params);
			}
			Cts_Cache::removeByTags(array('navigation'));
			$this->_redirect("/default/navigation/editrole/id/" . $role_id);
		} else {
