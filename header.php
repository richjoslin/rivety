...
{
	if (array_key_exists("debug_ips", $config['application']))
	{
		$debug_ips = explode(",", $config['application']['debug_ips']);
	}
	else
	{
		$debug_ips = array();
...
{
	set_include_path(get_include_path() . PATH_SEPARATOR . $default_dir . DIRECTORY_SEPARATOR . $subdir);
}

// set up default smarty plugins dir as a string
$smarty_plugins_dirs = $default_dir . DIRECTORY_SEPARATOR . 'smarty_plugins';

if ($isInstalled)
{
	$log_level = null;
	if (is_null(@$config['application']['log_level']))
	{
